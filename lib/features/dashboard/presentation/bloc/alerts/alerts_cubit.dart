...
class AlertsCubit extends Cubit<AlertsState> {
  ...
  Future<void> _markAlert(AlertEntity alert) async {
    ...
    final Set<int> marking =
        Set<int>.from(state.markingAlertIds)..add(alert.id);
    emit(state.copyWith(markingAlertIds: marking));

    final result =
        await _markAlertRead(MarkAlertReadParams(alert: alert));

    result.fold(
      (failure) {
        final Set<int> updatedMarking =
            Set<int>.from(state.markingAlertIds)..remove(alert.id);
        emit(
          state.copyWith(
      ...
      (_) {
        final List<AlertEntity> updatedAlerts = state.alerts
            .map(
              (AlertEntity item) => item.id == alert.id
  ...
  Future<void> deleteAlert(AlertEntity alert) async {
    ...
    final Set<int> deleting =
        Set<int>.from(state.deletingAlertIds)..add(alert.id);
    emit(state.copyWith(deletingAlertIds: deleting));

    final result =
        await _deleteAlert(DeleteAlertParams(alertId: alert.id));

    result.fold(
      (failure) {
        final Set<int> updatedDeleting =
            Set<int>.from(state.deletingAlertIds)..remove(alert.id);
        emit(
          state.copyWith(
      ...
      (_) {
        ...
        final Set<int> updatedExpanded =
            Set<int>.from(state.expandedAlertIds)..remove(alert.id);
        final Set<int> updatedMarking =
            Set<int>.from(state.markingAlertIds)..remove(alert.id);
        final Set<int> updatedDeleting =
            Set<int>.from(state.deletingAlertIds)..remove(alert.id);
