...
import 'package:oysloe_mobile/features/dashboard/presentation/bloc/terms_conditions/terms_conditions_state.dart';
...
class _TermsConditionsView extends StatelessWidget {
  const _TermsConditionsView();
    ...
    return Scaffold(
      ...
      body: BlocBuilder<TermsConditionsCubit, TermsConditionsState>(
        builder: (context, state) {
          if ((state.isLoading ||
                  state.status == TermsConditionsStatus.initial) &&
              !state.hasData) {
          ...
          if (state.hasError) {
            return _ErrorView(
              message: state.message ?? 'Unable to load terms & conditions.',
              onRetry: () => context
                  .read<TermsConditionsCubit>()
                  .fetch(forceRefresh: true),
            );
          }

          if (state.isSuccess && state.terms.isEmpty) {
            return const _EmptyTermsView();
          }

          final List<TermsConditionEntity> terms = state.terms;

          return RefreshIndicator(
            color: AppColors.blueGray374957,
            onRefresh: () =>
                context.read<TermsConditionsCubit>().fetch(forceRefresh: true),
            child: ListView(
              physics: const AlwaysScrollableScrollPhysics(),
              padding: EdgeInsets.all(4.w),
              children: [
                ...terms.map(
                  (TermsConditionEntity term) => Padding(
                    padding: EdgeInsets.only(bottom: 1.2.h),
                    child: _TermsTile(
                      term: term,
                      onTap: () => _showTermsSheet(context, term),
                    ),
                  ),
                ),
              ],
  ...
  static Future<void> _showTermsSheet(
    BuildContext context,
    TermsConditionEntity term,
  ) {
    final double fixedHeight = MediaQuery.of(context).size.height * 0.7;
    final bool isDark = Theme.of(context).brightness == Brightness.dark;
    final String dateLabel = _formatDate(term.date);
...
class _TermsTile extends StatelessWidget {
  ...
  final TermsConditionEntity term;
  final VoidCallback onTap;

  @override
  Widget build(BuildContext context) {
    final String dateLabel = _TermsConditionsView._formatDate(term.date);
